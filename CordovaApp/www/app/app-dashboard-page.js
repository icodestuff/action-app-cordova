...
(function (ActionAppCore, $) {
    ...
    ThisPage._onFirstLoad = function() {
        ...
        //tmpModuleTemplates[thisSiteSpecs.pageActionPrefix +":page-header"] = '<div class="ui menu" style="padding-bottom:0;margin-bottom:0"> <a appuse="tablinks" group="dash:tabs" item="home" action="showSubPage" class="active item">Dashboard</a> <a appuse="tablinks" group="dash:tabs" item="all" action="showSubPage" class="item">Dashboard</a> <a appuse="tablinks" group="dash:tabs" item="more" action="showSubPage" class="item">More</a>';
        tmpModuleTemplates[thisSiteSpecs.pageActionPrefix +":page-footer"] = '<h1>Dashboard Footer</h1>';
        var tmpAllBody = '';
        tmpAllBody += '<div appuse="cards" group="dash:tabs" item="home"> <div class="ui" style="padding-top:5px;margin-top:0px;min-height:300px;"> <div class="requests-outline"> <svg facet="dash:home-svg" viewBox="0 0 800 800" style="padding:0;margin:0;min-height:400px;background-color:black"> </div> <div class="requests-details"> <div class="ui menu flow middle" style=""> <div class=""> <div action="dash:runTest1" class="ui primary basic icon button"><i class="icon home"></i></div> </div> <div class="item"> <div action="dash:addBox" class="ui purple basic button">Add Box</div> </div> </div> <div facet="dash:home-out"></div> </div> </div> </div> <div appuse="cards" group="dash:tabs" item="all" class="hidden"> <div>Workspace: <b><span facet="dash:currentWorkspace">';
        tmpAllBody += '';
        tmpAllBody += '</span></b></div> <div class="ui menu flow middle" style=""> <div class="item"> <div action="dash:gotoMainWorkspace" class="ui primary basic icon button"><i class="icon home"></i></div> </div> <div class="item"> <div action="dash:workspaceTest" class="ui purple basic button">Workspace Testing</div> </div> </div> <div facet="dash:dashboard-out"></div> </div> <div appuse="cards" group="dash:tabs" item="more" class="hidden"> <div class="ui segment"> <p>Testing some values here</p> <div class="ui active"> <div class="ui loader"></div> </div> </div> </div> </div>';
    ...
    function pullSessionsFromLocal() {
        ...
        db.replicate.from(remoteDB).on('complete', function () {
            ...
            ThisApp.loadFacet('dash:home-out', "Replication Pull From DB Complete");
        }).on('error', function (err) {
            console.error("error in replication", err);
            ThisApp.loadFacet('dash:home-out', "Replication Error, see console");
        });
    }
